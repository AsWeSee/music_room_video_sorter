...
def check(ip1, ip2):
    ...
    # print(ip)
    try:
        urllib.request.urlopen('http://10.240.' + ip + ':13147/videostream.cgi?loginuse=' + camera_login + '&loginpas=' + camera_password, timeout=1)
    ...
    cap = cv2.VideoCapture('http://10.240.' + ip + ':13147/videostream.cgi?loginuse=' + camera_login + '&loginpas=' + camera_password)
    ret, frame = cap.read()
    cap.release()
    # brit = np.sum(frame)
    # print(brit)
